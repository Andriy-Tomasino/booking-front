...
try {
  const { initializeApp, getApps, getApp } = require('@react-native-firebase/app');
  const { getAuth } = require('@react-native-firebase/auth');
  app = getApps().length === 0 ? initializeApp(firebaseConfig) : getApp();
  auth = getAuth(app);
  console.log('[FirebaseNative] Initialized successfully');
} catch (error) {
  console.error('[FirebaseNative] Initialization error:', error);
  throw error;
}
